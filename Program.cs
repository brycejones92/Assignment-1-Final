...
namespace assignment1DIS
{
    class Program
    {
        ...
        private static int UniqueEmails(List<string> emails)
        {
            ...
            string domainPart;
            string localPart;
            string finished_email;

            // create a new list
            List<string> finished_email_list = new List<string>();
            ...
            try
            {
                ...
                foreach (string email in emails)
                {
                    ...
                    finished_email = localPart + "@" + domainPart;
                    
                    // adds final_email varriable to final_email_list
                    finished_email_list.Add(finished_email);
                }

                // declares variable res
                int des = (from x in finished_email_list select x).Distinct().Count();
        ...
        private static string DestCity(string[,] paths)
        {
            ...
            try
            {
                ...
                foreach(string r in destinationPoints)
                {
                    if (beginningPoints.Contains(r))
                    {
                        continue;
                    }
